...
X = pd.read_csv(os.path.join(anal_dir, "weather_L9G_mod.csv")).drop(["Date/Time (LST)"],axis=1)
Y = pd.read_csv(os.path.join(anal_dir, "power_L9G_mod.csv"))
...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y['TOTAL_CONSUMPTION'], test_size=1/(4*12), shuffle=False)
