...
def add_lags_to_weather_data(data: pd.DataFrame, lag: int):
    ...
    for i in range(1, lag + 1):
        ...
        data[f'Win _Speed_Lag_{i}'] = data['Wind Speed'].shift(i)
...
val_dir = data_path + "/validation_data"
...
if not os.path.exists(val_dir): # Create directory if it does not exist
    os.makedirs(val_dir)
...
fsa_map_path = "c:/Users/hanad/capstone_github/power-forecasting-capstone/fsa_finder/ontario_fsas.csv"
...
weather_data, power_data = asyncio.run(get_data_for_time_range(data_path, start_date, end_date, fsa, lat, lon, fsa_map))

# Save data (preform normalizations) to CSV
weather_data.to_csv(f'{val_dir}/weather_data_{fsa}_{start_date.strftime("%Y%m%d")}_{end_date.strftime("%Y%m%d")}.csv', index=False)
power_data.to_csv(f'{val_dir}/power_data_{fsa}_{start_date.strftime("%Y%m%d")}_{end_date.strftime("%Y%m%d")}.csv', index=False)
