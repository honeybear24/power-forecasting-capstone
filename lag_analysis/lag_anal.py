# Lag Analysis - Studying impact of that lags to weather variables have on forecasting accuracy of linear regression model
...
print("# of Lags, MAPE, MAE, R^2, MSE, RMSE")

# Cycle through the number of lags we want to look at
for i in range(0,25):

    # Import X data frame that has desired number of lags
    X = pd.read_csv(os.path.join(df_directory, f'weather_data_L9G_20180101_20231231_lagAnal_lag{i}.csv')).drop(["Date/Time (LST)"],axis=1)
